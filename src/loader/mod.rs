...
/// A program that has not been flattened.
/// This is part of a patch for the WASM frontend of the Cairo-M compiler.
...
pub fn load_wasm_unflattened<'a, S: Settings<'a>>(
    settings: S,
    wasm_file: &'a [u8],
) -> wasmparser::Result<UnflattenedProgram<'a, S>> {
    let parser = Parser::new(0);

    let mut ctx = UnflattenedProgram {
        c: CommonProgram {
            types: Vec::new(),
            func_types: Vec::new(),
            table_types: Vec::new(),
...
/// Generates a wrapper function for an imported function.
fn generate_imported_func_wrapper_unflattened<'a, S: Settings<'a>>(
    ctx: &UnflattenedProgram<'a, S>,
    function_idx: u32,
) -> BlocklessDag<'a> {
    let func_type = &ctx.c.get_func_type(function_idx).ty;
